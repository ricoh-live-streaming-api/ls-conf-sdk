...
|Name|Type|説明|
|:--|:--|:--|
|points|number[][]|ストロークを構成する座標の配列(※1)|
|isEnded|boolean|ストロークを書き終わったかどうか|
|option|[StrokeOption](#strokeoption)|ストロークのオプション<br>未指定の場合はデフォルト値となる|

(※1): 座標について。
- 範囲について
  - 360 映像は映像の左上を原点 (0, 0)、右下を (2048, 1024) とした時の座標が格納される
  - 通常映像は映像の左上を原点 (0, 0)、右下を (1, 1) とした時の座標が格納される
  - ストロークとして連続的な値を保持するため、映像サイズを超えた範囲の座標も含まれる
- 配列について
  - 360 映像に対しては座標情報のみの 2 次元配列 `[[x1, y1], [x2, y2], ...]` として扱う
  - 通常映像に対しては座標情報に加えて筆圧 (Pressure) の値が追加された 3 次元配列 `[[x1, y1, p1], [x2, y2, p2], ...]` として扱う
- 筆圧の値について
  - マウス等の筆圧のないインタフェースでの書き込み: `0.5` で固定
  - ペン, タッチ等の筆圧のあるインタフェースでの書き込み: `0 から 1.0 の間` で変化
- 座標を別クライアント側で描画する場合について
  - 通常映像に対して LSConf から取得したストロークを別クライアント（ClientSDK）で描画する際は、クライアントで描画している映像サイズと同じ大きさに拡大してストロークを表示する必要がある
...
```js
type ImageSource = URLImageSource | BlobImageSource;
```

画像ファイルの指定方法に応じて以下の通り使用する。
- URLImageSource : 画像ファイルの URL を指定する場合に使用する
- BlobImageSource : 画像ファイルの Blob データを指定する場合に使用する

#### URLImageSource

画像ファイルのソース情報を表す。画像ファイルを URL で指定する場合に使用する。

```js
type URLImageSource = { url: string } & ImageSourceCommon;
```

以下の url に画像ファイルのソース情報の共通項目として ImageSourceCommon を加える。
...
画像ファイルのソース情報を表す。画像ファイルを Blob データで指定する場合に使用する。

```js
type BlobImageSource = { blob: Blob } & ImageSourceCommon;
```

以下の blob に画像ファイルのソース情報の共通項目として ImageSourceCommon を加える。

|Name|Type|説明|
|:--|:--|:--|
| blob | Blob | 画像ファイルのBlobデータ(※) |

(※) Blob データについて。
- Blob データには `getCaptureImage(subView, options)` で取得した Blob 渡す

#### ImageSourceCommon

画像ファイルのソース情報の共通項目。URLImageSource および BlobImageSource の共通項目として使用する。

```js
type ImageSourceCommon = {
  connectionId: IDString;
  label: string;
  isTheta: boolean;
};
...
LSConf のエラー種別は（[LSConfError](https://livestreaming.ricoh/docs/lsconf-error-specification/#%E3%82%A8%E3%83%A9%E3%83%BC%E4%B8%80%E8%A6%A7)）にて定義する。<br>
以下のエラーはエラーデータを取得できる。
...
`theme` による LSConf のデザインの詳細なカスタマイズ方法については[RICOH Live Streaming Conference デザインカスタマイズガイド](https://livestreaming.ricoh/docs/lsconf-design-customize-guide/)を参照ください。
...
| `player.isHiddenVideoControlBar` | boolean | false | ◯ | [VideoControlBar](https://livestreaming.ricoh/docs/lsconf-function/#videocontrolbar) を非表示にする|
| `player.localFileHeader` | Object | | ◯ | ローカルファイル指定のヘッダーオブジェクト（`createPlayer` で `sources` が `null` または `undefined` の場合のみ表示） |
| `player.localFileHeader.title` | string | "Player" | ◯ | ヘッダーに表示するタイトル |
...
RICOH Live Streaming で録画した動画を利用した、Player コンポーネントの iframe を生成する。<br>
`sources` が `null` または `undefined` の場合は、ローカルファイルプレイヤーが表示される。
...
|:--|:--|:--|
|parentElement|HTMLElement|iframeが指定されたElementの子として追加される|
|sources|VideoSource[] \| string \| null \| undefined |Playerに追加したい [VideoSource](#VideoSource) の配列<br>または動画ファイルのソース情報が定義されたjsonファイルのURL(※1)<br>`null` または `undefined` を明示的に指定するとローカルファイルプレイヤーが表示される(※2)|
|parameters|`Partial<CreateParameters>`|[`CreateParameters`](#CreateParameters) を設定する|
...
- プレイヤーに動画を開くボタンと 360 表示トグルが表示される
- 動画を開くボタンを押すとファイル選択ダイアログが開き、ローカルの動画ファイルを 1 件選択できる
  - 選択可能なファイルは[クラウド録画](https://livestreaming.ricoh/docs/record-developer-guide/)、および[ローカル録画](https://livestreaming.ricoh/docs/lsconf-function/#%E9%8C%B2%E7%94%BB%E8%A8%AD%E5%AE%9A%E3%83%80%E3%82%A4%E3%82%A2%E3%83%AD%E3%82%B0)でサポートされている動画ファイル形式・コーデックのファイルが対象となる
- 選択したファイルがプレイヤーで再生される
- 360 表示トグル（初期値：ON）によって、 360 動画として再生するかどうかを切り替えられる
- `CreateParameters`の`player.localFilePlayer.title`でタイトルを設定できる
...
SubView の一覧を取得する。<br>
他拠点や自拠点の画面共有の SubView は映像トラックの追加が完了後（`remoteTrackAdded`, `screenShareMediaOpen` のイベント後）に取得可能となる。
...
| connectionId | IDString \| undefined  | [ローカル録画] 録画を行った拠点のconnection_id<br>[クラウド録画] 未指定 |
...
- 引数
  - require
    ...
    - options
- 返り値
  - `Promise<{ blob: Blob; rotationVector: RotationVector | null }>`
- エラー（[LSConfError](https://livestreaming.ricoh/docs/lsconf-error-specification/#%E3%82%A8%E3%83%A9%E3%83%BC%E4%B8%80%E8%A6%A7)）
...
|Name|Type|説明|
|:--|:--|:--|
| blob | Blob | 取得した静止画のBlobデータ |
| rotationVector | [RotationVector](#rotationvector) | 撮影前の THETA（Android 標準の [SensorManager](https://developer.android.com/reference/android/hardware/SensorManager)）から取得できる [回転ベクトルセンサー](https://developer.android.com/guide/topics/sensors/sensors_position?hl=ja) の値。天頂補正していない場合は `null` を返す |
...
画像ファイル（URL または Blob データ）を指定して SubView に追加する。<br>
<!-- textlint-disable ja-technical-writing/sentence-length -->
...
利用環境が H.265 のコーデックの映像の送受信をサポートしているかを確認する。<br>
...
- 返り値
  - `Promise<boolean>`
    - `true`: H.265 に対応している
    - `false`: H.265 に対応していない
- エラー（[LSConfError](https://livestreaming.ricoh/docs/lsconf-error-specification/#%E3%82%A8%E3%83%A9%E3%83%BC%E4%B8%80%E8%A6%A7)）
  - CheckH265SupportFailed
  - CheckH265SupportError
...
エラーが発生した。<br>
エラーの詳細な仕様については [エラー仕様](https://livestreaming.ricoh/docs/lsconf-error-specification) を参照してください。
